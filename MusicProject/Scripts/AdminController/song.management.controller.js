appMusic.controller('SongManaController', function ($scope, $rootScope, $http, $window, $filter, NgTableParams, Upload) {
    ...
    $scope.addeditSongSave = function () {
        ...
        if ($scope.feature == 'Thêm bài hát') {
            ...
            if ($scope.Song.Album) {
                songNewClone.AlbumID = $scope.Song.Album.AlbumID
                delete songNewClone.Album
            }
            let collectionneedAdd
            if (songNewClone.Collections && songNewClone.Collections.length > 0) {
                collectionneedAdd = songNewClone.Collections.map(({ CollectionName, ...rest }) => {
                    rest.SongID = songNewClone.SongID
                    return rest
                })
            }
            let artistneedAdd
            if (songNewClone.Artists && songNewClone.Artists.length > 0) {
                artistneedAdd = songNewClone.Artists.map(({ ArtistName, Image, ...rest }) => {
                    rest.SongID = songNewClone.SongID
                    return rest
                })
            }
            songNewClone.Collections = collectionneedAdd
            songNewClone.Artists = artistneedAdd
            ...
            if ($scope.songs[songIndex].Collections && $scope.songs[songIndex].Collections.length > 0) { // Nếu thằng ban đầu (thằng gốc trong mảng) có ít nhất 1 phần tử
                collectionneedEdit = songNewClone.Collections.map(({ CollectionName, ...rest }) => { // map nó thằng clone (thằng mình nhập) nếu nó có tồn tại trong thằng gốc thì sửa, ko thì thêm
                    rest.SongID = songNewClone.SongID
                    rest.status = $scope.songs[songIndex].Collections.findIndex(c => c.CollectionID == rest.CollectionID) == -1 ? 0 : 1
                    return rest
                })
                $scope.songs[songIndex].Collections.forEach(({ CollectionName, ...rest }) => {//Nếu thằng gốc trong mảng có phần tử mà thằng clone kia ko có thì xóa
                    if (collectionneedEdit.findIndex(c => c.CollectionID == rest.CollectionID) == -1) {
                        rest.SongID = songNewClone.SongID
                        rest.status = 2
                        collectionneedEdit.push(rest)
                    }
                })
                ...
                if (songNewClone.Collections && songNewClone.Collections.length > 0) {//Nếu thằng clone có collection thì cho về dạng thêm
                    collectionneedEdit = songNewClone.Collections.map(({ CollectionName, ...rest }) => {
                        rest.SongID = songNewClone.SongID
                        rest.status = 0
                        return rest
                    })
            ...
            if ($scope.songs[songIndex].Artists && $scope.songs[songIndex].Artists.length > 0) {
                artistneedEdit = songNewClone.Artists.map(({ ArtistName, Image, ...rest }) => {
                    rest.SongID = songNewClone.SongID
                    rest.status = $scope.songs[songIndex].Artists.findIndex(a => a.ArtistID == rest.ArtistID) == -1 ? 0 : 1
                    return rest
                })
                $scope.songs[songIndex].Artists.forEach(({ ArtistName, Image, ...rest }) => {
                    if (artistneedEdit.findIndex(c => c.ArtistID == rest.ArtistID) == -1) {
                        rest.SongID = songNewClone.SongID
                        rest.status = 2
                        artistneedEdit.push(rest)
                    }
                })
            } else {
                if (songNewClone.Artists && songNewClone.Artists.length > 0) {
    ...
    $scope.agreetodeleteSong = function () {
        $http({
            ...
            if (!res.data) {
                let songIndex = $scope.songs.findIndex(s => s.SongID == $scope.songIDneedDelete)
